...
const emptyItems = computed(() => items.value.reduce<Array<{x: number,y: number}>>( (acc, row, y) => {
  row.forEach((item, x) => {
    if (item === null) {
      acc.push({x, y})
    }
...
const addRandomItem = () => {
  const randIndex = Math.floor(Math.random() * emptyItems.value.length)

  items.value[emptyItems.value[randIndex].y][emptyItems.value[randIndex].x] = 2
}
...
const removeSpaces = (input: Array<number | null>) => {
  return input.filter((input) => input !== null) as Array<number | null>
}
...
const isEqualArrs = (arr1: Array<number | null>[], arr2: Array<number | null>[]) => {
