...
describe("processArrOnMoveLeft", () => {
  it("test arr", () => {
    ...
    expect(processArrOnMoveLeft(input).map((item) => item.value)).toEqual([4, null, null, null]);
  });

  it("test 2 arr", () => {
    const input = [2, 2, 2, 2].map((value) => ({ id: 2, value }));

    expect(processArrOnMoveLeft(input).map((item) => item.value)).toEqual([4, 4, null, null]);
  });
